...
.pill.blue { background: #EBF2FF; color: #2563EB; }
.pill.orange { background: #FFF4E6; color: #D97706; }
.pill.red { background: #FFF0EF; color: #DC2626; }
.pill.green { background: #EDFAF1; color: #16A34A; }
...
with col1:
    ...
    bar_colors = [action_color_map.get(a, '#CCC') for a in action_counts.index]
    ...
    ax.set_xlabel("Count")
    ax.set_title("Action Type Distribution", fontsize=9, color='#444', pad=10, loc='left')
    ax.spines['bottom'].set_visible(True)
    ax.spines['bottom'].set_color('#E8E5DE')
...
with col2:
    ...
    ax.set_title("Hesitation Rate by Node", fontsize=9, color='#444', pad=10, loc='left')
    ax.spines['bottom'].set_visible(True)
    ax.spines['bottom'].set_color('#E8E5DE')
...
with col3:
    ...
    node_order = ['Start','Check SOP','Review Terminology','Interpret Step',
                  'Check Prior Record','Execute Step','Verify Result','Document Entry',
                  'Final Review','End']
    pos = {}
    ...
    for u, v, data in G.edges(data=True):
        ...
        edge_colors.append(COLORS.get(act, '#CCC'))
    ...
    ax.set_title("Cognitive Pathways Network", fontsize=9.5, color='#444',
                 pad=12, loc='left', fontweight='500')
    ax.axis('off')
    fig.tight_layout()
    st.pyplot(fig)
    plt.close()
...
with col4:
    ...
    ax.set_title("Time Distribution", fontsize=9, color='#444', pad=8, loc='left')
    ax.spines['bottom'].set_visible(True)
    ax.spines['bottom'].set_color('#E8E5DE')
    fig.tight_layout()
    st.pyplot(fig)
    ...
    # Interruption vs hesitation summary
    total_f = len(fdf)
    hes_n = fdf["hesitation_flag"].sum()
    int_n = fdf["interruption_flag"].sum()

    st.markdown(f"""
    <div style="background:#FAFAF8; border-radius:12px; padding:1rem; border:1px solid #E8E5DE;">
      <div class="metric-label" style="margin-bottom:0.8rem;">Cognitive load summary</div>
      <div style="margin-bottom:0.6rem;">
        <span class="pill red">Hesitation</span>
        <span style="font-family:'DM Mono',monospace; font-size:0.9rem;">{hes_n} events · {round(hes_n/total_f*100,1)}%</span>
      </div>
      <div style="margin-bottom:0.6rem;">
        <span class="pill orange">Interruption</span>
        <span style="font-family:'DM Mono',monospace; font-size:0.9rem;">{int_n} events · {round(int_n/total_f*100,1)}%</span>
      </div>
      <div>
        <span class="pill">Avg time/step</span>
        <span style="font-family:'DM Mono',monospace; font-size:0.9rem;">{round(fdf['time_spent_sec'].mean(),1)}s</span>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# pre-compute to avoid f-string formatting issues
_crossref = int(crossref_n)
_loopback = int(loopback_n)
_hes_rate = round(df['hesitation_flag'].mean() * 100, 1)

arg_html = (
    '<div style="background:#FFFFFF; border-radius:16px; padding:1.5rem 2rem; border:1px solid #E8E5DE;">'
    '<div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1.5rem;">'

    '<div>'
    '<div class="metric-label" style="margin-bottom:0.6rem; color:#3D7EFF;">Working memory overload</div>'
    '<div style="font-family:\'DM Mono\',monospace; font-size:1.4rem; color:#1A1A1A; margin-bottom:0.3rem;">' + str(_crossref) + '</div>'
    '<div style="font-size:0.8rem; color:#666;">cross-reference transitions — '
    ...
    '<div style="font-family:\'DM Mono\',monospace; font-size:1.4rem; color:#1A1A1A; margin-bottom:0.3rem;">' + str(_loopback) + '</div>'
    '<div style="font-size:0.8rem; color:#666;">loopback transitions — '
    ...
    '<div style="font-family:\'DM Mono\',monospace; font-size:1.4rem; color:#1A1A1A; margin-bottom:0.3rem;">' + str(_hes_rate) + '%</div>'
    '<div style="font-size:0.8rem; color:#666;">of all transitions flagged as hesitation — '
    ...
    '</div>'
    '<div style="margin-top:1.25rem; padding-top:1.25rem; border-top:1px solid #F0EDE6; '
    'font-size:0.85rem; color:#555; font-style:italic; line-height:1.6;">'
...
with st.expander("View raw data"):
    st.dataframe(
        fdf.style.apply(
            lambda col: ['background-color: #FFF0EF' if v == 1 else '' for v in col]
